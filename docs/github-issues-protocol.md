...
### **Стандартный формат заголовков:**
```
[TYPE][PRIORITY] Краткое описание задачи
```

**Обязательно указывать ТИП первым, приоритет - вторым:**
- `[DOCS][CRITICAL] Обновить архитектурную документацию`  
- `[BUG][HIGH] FMP API Router падает при timeout`
- `[FEATURE][MEDIUM] Добавить Slack уведомления`
- `[TEST][LOW] Создать regression тесты для edge cases`
...
**⚡ ВАЖНО:** Всегда указывайте ТИП первым, чтобы было понятно о чем Issue, даже если это критическая задача.
...
**Примеры:**
- `[FEATURE][MEDIUM] Создать workflow для анализа криптовалют`
- `[ENHANCEMENT][HIGH] Добавить error handling в FMP API Router`
- `[WORKFLOW][LOW] Миграция AI Deepseek v1.2 в PROD`
...
**Примеры:**
- `[BUG][CRITICAL] FMP API Router возвращает 500 для Insider.Trading.Latest`
- `[BUG][HIGH] Test Orchestrator не собирает результаты от AI Deepseek`
...
**Примеры:**
- `[TEST][MEDIUM] Создать regression тесты для FMP API изменений`
- `[TEST][HIGH] Протестировать AI Deepseek с новыми prompts`
...
**Примеры:**
- `[DOCS][CRITICAL] Обновить схему Test Orchestrator архитектуры`
- `[DOCS][MEDIUM] Создать руководство по миграции DEV→PROD`
...
**Примеры:**
- `[ENHANCEMENT][HIGH] Реструктурировать папки workflows`
- `[FEATURE][MEDIUM] Добавить Slack MCP для уведомлений`
...
5. **Следуем naming convention**: `[TYPE][PRIORITY] Description`
